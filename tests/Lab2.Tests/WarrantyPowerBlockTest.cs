...
public class WarrantyPowerBlockTest
{
    private readonly ComputerDirector _computerDirector;
    public WarrantyPowerBlockTest()
    {
        ...
        var powerBlock3 = new PowerBlock("Aerocool Value Plus 300W", 300);
        var powerBlock4 = new PowerBlock("Thermaltake 200W", 200);

        storage.AddPowerBlock(powerBlock1);
        storage.AddPowerBlock(powerBlock2);
        storage.AddPowerBlock(powerBlock3);
        storage.AddPowerBlock(powerBlock4);
    ...
    [Theory]
    [InlineData("Aerocool Value Plus 350W")]
    [InlineData("Aerocool Value Plus 300W")]
    [InlineData("Thermaltake 200W")]
    public void WarrantyPowerBlock(string powerBlockName)
    {
        GetDefaultDirector();
        _computerDirector.SetPowerBlock(powerBlockName);
    ...
    private void GetDefaultDirector()
    {
        _computerDirector.SetMotherboard("MSI MPG Z490 GAMING PLUS")
            ...
            .SetGpu("Asus GeForce RTX 4080 ROG Strix")
            .SetHdd("Seagate BarraCuda Compute 2.5")
            .SetSsd("Samsung 970 EVO Plus M.2")
            .SetPowerBlock("Aerocool Value Plus 300W")
