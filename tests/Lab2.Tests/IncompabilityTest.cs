...
public class IncompabilityTest
{
    private readonly ComputerDirector _computerDirector;
    public IncompabilityTest()
    {
    ...
    [Theory]
    [InlineData("AMD Ryzen 7 5700G", "MSI MPG Z490 GAMING PLUS")]
    [InlineData("Intel Core i5 12400F", "MSI MPG Z490 GAMING PLUS")]
    public void IncompabilityCpu(string cpuName, string motherboardName)
    {
        GetDefaultDirector();
        _computerDirector.SetCpu(cpuName)
            .SetMotherboard(motherboardName);
        Action act = () => _computerDirector.Construct();
